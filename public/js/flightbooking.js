document.addEventListener('DOMContentLoaded', function() {
    ...
    const flightGrid = document.querySelector('.flight-grid');
    if (flightGrid) {
        flightGrid.addEventListener('click', function(e) {
            if (e.target && e.target.classList.contains('book-now-btn')) {
                const card = e.target.closest('.flight-card');
                if (card) {
                    const airline = card.querySelector('.airline-info span').textContent.trim();
                    const depCity = card.querySelector('.departure p').textContent.trim();
                    const arrCity = card.querySelector('.arrival p').textContent.trim();
                    const priceText = card.querySelector('.price').textContent.trim();
                    const amount = priceText.replace(/[^\d]/g, '');
                    
                    const bookingDetails = {
                        service: `${airline} Flight (${depCity} -> ${arrCity})`,
                        duration: 'One-Way Flight',
                        amount: amount
                    };
